...
public class BluetoothChatService {
    ...
    public static BluetoothChatService getInstance() throws GhostPasswordException {
        if(null == mInstance){
            try {
                ...
                Toast.makeText(GhostPassword.getContext(),e.getMessage().toString(),Toast.LENGTH_LONG).show();
                throw(e);
            }
        }
        return mInstance;
    }
    public void writeString(String s) throws IOException, GhostPasswordException {
        // Wrap the string with a 'string to type' identifier for the arduino
        s = ':' + s + ':';
        mInstance.write(s.getBytes());
        Log.d("ChatLog","Wrote string");
    }
    public void writeQR(String s) throws IOException, GhostPasswordException {
        // Wrap the string with a QR identifier for the arduino
        s = '-' + s + ':';
        mInstance.write(s.getBytes());
                ...
                case Constants.MESSAGE_STATE_CHANGE:
                    switch (msg.arg1) {
                        case BluetoothChatService.STATE_CONNECTED:
                            ...
                            Toast.makeText(GhostPassword.getContext(),"Connected to device",Toast.LENGTH_LONG).show();
                            break;
                        case BluetoothChatService.STATE_CONNECTING:
                            //setStatus(R.string.title_connecting);
                            Toast.makeText(GhostPassword.getContext(),"Connecting to device",Toast.LENGTH_SHORT).show();
                            break;
                        case BluetoothChatService.STATE_LISTEN:
                        case BluetoothChatService.STATE_NONE:
                            //setStatus(R.string.title_not_connected);
                            Toast.makeText(GhostPassword.getContext(),"Not connected",Toast.LENGTH_SHORT).show();
                            break;
                    }
                    break;
     ...
     *
     */
    public BluetoothChatService() throws GhostPasswordException {
        ...
        if (mAdapter != null) {
            if (mAdapter.isEnabled()) {
                ...
                if (bondedDevices.size() > 0) {
                    ...
                    if(devices.size() == 0){
                        throw new GhostPasswordException("No paired Ghost Test messagePassword devices", new Throwable("Bluetooth"));
                    }
